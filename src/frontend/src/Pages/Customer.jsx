...
export default function Customer() {
  ...
  const handleUpdateUser = async (event) => {
    ...
    const updatedData = {
      ...
      password: document.getElementById("password").value || customer.password,
                ...
                updating.loading ? (
                  <div className="bg-yellow-100 text-yellow-900 px-4 py-3 rounded-md mb-4">
                    ...
                    <div>
                      ...
                      {updating.error && updating.error.messages && (
                        <div className="bg-red-100 text-red-900 px-4 py-3 rounded-md mb-4">
                          {/* {updating.error.messages.map((message, index) => (
                            <p key={index}>{message}</p>
                          ))} */}
                          <div>{updating.messages}</div>
                        </div>
                       )} 
                    </div>
                  )
  ...
  <div className="mb-4">
    ...
    <input
      ...
      id="dateOfBirth"
      className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
      defaultValue={new Date(customer["date_of_birth"]).toISOString().split('T')[0]}
      required
    />
